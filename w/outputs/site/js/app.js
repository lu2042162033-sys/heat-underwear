...
(function () {
  ...
  function ghApi(path, opts) {
    ...
    return fetch(path, init).then(function (res) {
      ...
      return res.json().then(function (data) {
        ...
        var ok204 = res.status >= 200 && res.status < 300;
        var out = { ok: ok204, status: res.status, data: null };
        setDebug('请求返回 ' + res.status + '（空响应体，按 ' + (ok204 ? '成功' : '失败') + ' 处理）');
        return out;
      });
    }).catch(function (e) {
  ...
  function filtered() {
    ...
    } else if (s === 'price-asc' || s === 'price-desc') {
      var price = function (p) { return Math.max(toNum(p.unitPrice), toNum(p.dozenPrice)); };
      list = list.slice().sort(function (a, b) { return s === 'price-asc' ? price(a) - price(b) : price(b) - price(a); });
    }
    return list;
  }
  ...
  function renderNav() {
    ...
    var sortSel = $('cat-sort');
    var sortLabel = $('cat-sort-label');
    ...
    if (sortSel) {
      ...
      sortOpts.forEach(function (pair) {
  ...
  function openCatDrawer() {
    state.menuOpen = true;
    var d = $('cat-drawer');
    if (d) { d.hidden = false; requestAnimationFrame(function () { d.classList.add('open'); }); }
  }
  function closeCatDrawer() {
    state.menuOpen = false;
    var d = $('cat-drawer');
    if (d) { d.classList.remove('open'); setTimeout(function () { d.hidden = true; }, 220); }
  }
  function card(p) {
    ...
    var priced = toNum(p.unitPrice) > 0 || toNum(p.dozenPrice) > 0;
    var qa = el('button', 'quick-add' + (priced ? '' : ' disabled'), '+');
    qa.type = 'button';
    qa.setAttribute('aria-label', t('addToCart'));
    qa.setAttribute('aria-disabled', priced ? 'false' : 'true');
    if (priced) {
      qa.addEventListener('click', function (e) {
        e.stopPropagation();
        addToCart(p, 1);
        showToast(t('cartAdded'), 'success', 1800);
      });
    }
    media.appendChild(qa);
  ...
  function renderGrid() {
    ...
    list.forEach(function (p, idx) {
      var c = card(p);
      c.style.animationDelay = (Math.min(idx, 12) * 14) + 'ms';
      grid.appendChild(c);
    });
  ...
  function addToCart(p, qty) {
    var q = Math.max(1, Math.min(99, toNum(qty) || 1));
    var mode = "unit";
    for (var i = 0; i < state.cart.length; i++) {
      if (String(state.cart[i].id) === String(p.id) && state.cart[i].mode === mode) {
        state.cart[i].qty = Math.min(99, toNum(state.cart[i].qty) + q);
        saveCart(state.cart);
        return;
      }
    }
  ...
  function cartLinePrice(item) {
    var qty = toNum(item.qty);
    if (item.mode === "dozen") return toNum(item.dozenPrice) * qty;
    return toNum(item.unitPrice) * qty;
  }
  function cartTotal() {
    var s = 0;
    state.cart.forEach(function (it) { s += cartLinePrice(it); });
    return Math.round(s);
  }
  function fmtMoney(n) {
    var sym = CFG.currencySymbol || "$";
    return sym + " " + Number(Math.round(n)).toLocaleString("en-US");
  }
  function renderCartPanel() {
    ...
    state.cart.forEach(function (it) {
      ...
      var bU = el("button", "mode-btn" + (it.mode === "unit" ? " active" : ""), t("cartByUnit"));
      var bD = el("button", "mode-btn" + (it.mode === "dozen" ? " active" : ""), t("cartByDozen"));
      bU.type = "button"; bD.type = "button";
      bU.addEventListener("click", function () { it.mode = "unit"; it.qty = 1; saveCart(state.cart); });
      ...
      var hint = el("div", "cart-line-hint", it.mode === "dozen"
        ? it.qty + " " + t("cartByDozen") + " × " + fmtMoney(toNum(it.dozenPrice))
      ...
      bDel.addEventListener("click", function () {
        for (var i = 0; i < state.cart.length; i++) {
          if (String(state.cart[i].id) === String(it.id)) { state.cart.splice(i, 1); break; }
        }
        saveCart(state.cart);
      });
  ...
  function exportCartPdf() {
    ...
    var css = "body{font-family:Microsoft YaHei,PingFang SC,sans-serif;padding:32px;color:#222;}" +
      "h1{font-size:22px;margin:0 0 4px;}p{margin:2px 0;font-size:12px;color:#555;}" +
      "table{width:100%;border-collapse:collapse;margin-top:18px;font-size:13px;}" +
      "th,td{border:1px solid #ccc;padding:8px 10px;text-align:left;}" +
      "th{background:#f5f5f5;}td.r{text-align:right;}.total{font-weight:bold;font-size:15px;}";
    var html = "<html><head><meta charset=\"utf-8\"><title>" + t("cartExportTitle") + "</title><style>" + css + "</style></head><body>" +
      "<h1>" + (CFG.title || "") + "</h1>" +
      "<p>" + (CFG.storeLine || "") + "</p>" +
  ...
  function saveForm() {
    ...
    if (!code || !cat) { alert(t('required')); return; }
    var obj = normalize({
      code: code,
  ...
  function init() {
    ...
    document.addEventListener('keydown', function (e) {
      if (e.key === 'Escape') {
        ...
        else if (!$('cart-overlay').hidden) closeCart();
        else if (state.menuOpen) closeCatDrawer();
    ...
    var catMenuBtn = $('cat-menu-btn');
    if (catMenuBtn) {
      catMenuBtn.addEventListener('click', function () {
        if (state.menuOpen) closeCatDrawer(); else openCatDrawer();
      });
    }
    var catDrawerClose = $('cat-drawer-close');
    if (catDrawerClose) catDrawerClose.addEventListener('click', closeCatDrawer);
    var catSort = $('cat-sort');
    if (catSort) catSort.addEventListener('change', function () {
      state.sort = catSort.value;
      renderGrid();
    });
    document.addEventListener('click', function (e) {
      if (state.menuOpen && e.target.closest && !e.target.closest('#cat-drawer') && !e.target.closest('#cat-menu-btn')) {
        closeCatDrawer();
      }
    });
    ...
    var bCart = $('btn-cart');
    if (bCart) bCart.addEventListener('click', openCart);
    var cartClose = $('cart-close');
    if (cartClose) cartClose.addEventListener('click', closeCart);
    var cartBack = $('btn-cart-back');
    ...
    if (btnAddCart) {
      btnAddCart.addEventListener('click', function () {
        if (!state.lbProduct) return;
        addToCart(state.lbProduct, $('lb-qty').value);
        alert(t('cartAdded'));
      });
    }
    try {
    ...
    function finish() {
      ...
      window.__HEAT = {
        ...
        renderAll: renderAll,
        cart: { loadCart: loadCart, saveCart: saveCart, addToCart: addToCart, cartTotal: cartTotal, renderCartPanel: renderCartPanel, exportCartPdf: exportCartPdf }
      };
    }
